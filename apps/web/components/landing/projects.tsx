    ...
    return (
        <article
            ...
            className={cn(
                ...
                "cursor-pointer border-[#111318]/18 hover:border-[#111318]/70 focus-visible:border-[#111318]/70",
                "focus-visible:ring-2 focus-visible:ring-[#B8792E] focus-visible:ring-offset-4 focus-visible:ring-offset-[#F4EFE8] active:border-[#B8792E]/60",
...
function ProjectShowcaseOverlay({
    ...
    return (
        <motion.div
            className="fixed inset-0 z-[9999] bg-[#111318]/45 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] backdrop-blur-[2px]"
            role="dialog"
            aria-modal="true"
            aria-labelledby={`${project.id}-showcase-title`}
            onClick={onClose}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
        ...
        >
            <motion.div
                className="mx-auto flex max-h-[calc(100dvh-2rem)] w-full max-w-[1100px] flex-col overflow-hidden border border-[#111318]/18 bg-[#F4EFE8] shadow-[0_30px_90px_rgba(17,19,24,0.28)]"
            ...
            >
                ...
                <div className="min-h-0 flex-1 overscroll-contain overflow-y-auto px-4 py-5 sm:px-6">
                    <h3
                        id={`${project.id}-showcase-title`}
                        dir={textDirection}
                        className={cn(
                            "max-w-[28rem] text-[clamp(2rem,4vw,2.8rem)] font-medium leading-[0.98] text-[#111318]",
                    ...
                    <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,0.75fr)]">
                        <div className="grid gap-5">
                            <div className="overflow-hidden border border-[#111318]/16 bg-[#F4EFE8] shadow-[0_26px_90px_rgba(17,19,24,0.12)]">
                                <ShowcaseHeader
                                    eyebrow={project.showcase.lead.eyebrow}
                                    textDirection={textDirection}
                                />

                                <div className="grid gap-4 p-4 lg:grid-cols-[0.42fr_0.58fr]">
                                    <ShowcaseCopyBlock
                                        copy={project.showcase.lead}
                                        textDirection={textDirection}
                                    />
                                ...
                                <div className="grid gap-4 border-t border-[#111318]/12 sm:grid-cols-2 sm:gap-0">
                                    <ShowcaseCopyBlock
                                        copy={project.showcase.problem}
                                        textDirection={textDirection}
                                    />
                                    <div className="sm:border-l sm:border-[#111318]/12">
                                        <ShowcaseCopyBlock
                                            copy={project.showcase.system}
                                            textDirection={textDirection}
                                        />
                        ...
                        <div className="overflow-hidden border border-[#111318]/16 bg-[#F4EFE8] shadow-[0_26px_90px_rgba(17,19,24,0.12)]">
                            ...
                            <div className="p-4">
                                <ProjectImage
                                    image={project.showcase.mobileImage}
                                    className="mx-auto aspect-[9/16] w-full max-w-[18rem] border border-[#111318]/10 bg-[#F4EFE8] sm:max-w-[20rem] lg:max-w-none"
                                    sizes="(min-width: 1024px) 22vw, (min-width: 640px) 55vw, 80vw"
                                    unavailableLabel={copy.imageUnavailable}
                                    fit="cover"
                                />
                            </div>
...
export default function Projects({ copy, textDirection }: ProjectsProps) {
    ...
    const [openProjectId, setOpenProjectId] = useState<Project["id"] | null>(
        null,
    );
    ...
    useEffect(() => {
        if (!openProjectId || typeof window === "undefined" || !isMobileViewport()) {
            return;
        }
    ...
    }, [openProjectId, closeProjectShowcase]);

    useEffect(() => {
        if (!openProjectId || typeof window === "undefined") {
            return;
        }
    ...
    const openProject = useMemo<Project | null>(() => {
        if (!openProjectId) {
            return null;
        }
